...
class Mono_Flat_GLM(nn.Module):
    def __init__(self, sub_no, E_no, I_no, T_no, device):
        ...
        ### Synapse Parameters ###
        self.W_e = nn.Parameter(torch.rand(self.sub_no, self.T_no)*0.01, requires_grad=True)
        self.W_i = nn.Parameter(torch.rand(self.sub_no, self.T_no)*(-0.01), requires_grad=True)
    ...
    def forward(self, S_e, S_i, temp, test):
        ...
        final = torch.tanh(root_in + self.Theta[0]) * torch.exp(self.W_sub[0]) + self.V_o
