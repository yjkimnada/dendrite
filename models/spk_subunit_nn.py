...
class Spk_Subunit_NN(nn.Module):
    def __init__(self, C_syn_e, C_syn_i, T_no, H_no, device):
        ...
        #for h in range(self.sub_no):
            #self.ff.append(nn.Sequential(nn.Linear(self.H_no, 1, bias=False)))
        ...
        self.W_ff = nn.Parameter(torch.randn(self.sub_no, self.H_no)*0.1, requires_grad=True)
    ...
    def forward(self, S_e, S_i):
        ...
        for t in range(T_data):
            ...
            conv_in = torch.sum(conv_out[:,:,t]*self.W_ff, 1) #(sub_no)
            sub_in = hist_in + conv_in + self.threshold
