...
class GRU_Stacked(nn.Module):
    def __init__(self, C_syn_e, C_syn_i, H_no, device):
        ...
        self.soma_gru = nn.GRU(self.sub_no, self.H_no, batch_first=True)
        self.soma_linear = nn.Linear(self.H_no, 1)
    ...
    def forward(self, S_e, S_i):
        ...
        final_gru, _ = self.soma_gru(sub_out)
        final_lin = self.soma_linear(final_gru.reshape(-1,self.H_no)).reshape(batch_size, T_data)
        final = final_lin + self.V_o
