...
class VAE_Hist_GLM3(nn.Module):
    def __init__(self, C_den, C_syn_e, C_syn_i, T_no, device):
        ...
        #self.W_syn_2 = nn.Parameter(torch.zeros(self.sub_no, self.plex_no, self.cos_basis_no) , requires_grad=True)
        
        self.W_syn_1 = nn.Parameter(torch.randn(self.sub_no*self.plex_no, 2, self.T_no)*0.01 , requires_grad=True)
        self.W_syn_2 = nn.Parameter(torch.randn(self.sub_no, self.plex_no, self.T_no)*0.01 , requires_grad=True)
    ...
    def spike_convolve(self, S_e, S_i):
        ...
        conv2 = F.conv1d(in_pad_2, conv2_weight, groups=self.sub_no).squeeze(0).T
        syn = conv2
