...
class Cos_TCN(nn.Module):
    def __init__(self, E_no, I_no, T_no, hid_no, layer_no, device):
        super().__init__()
        ...
        self.W_e_layer1 = nn.Parameter(torch.randn(self.hid_no*self.E_no , self.cos_basis_no)*0.01)        
        self.W_i_layer1 = nn.Parameter(torch.randn(self.hid_no*self.I_no , self.cos_basis_no)*0.01)
    ...
    def forward(self, S_e, S_i):
        ...
        pad_syn_e = torch.zeros(batch, T_data + self.T_no-1, self.E_no).to(self.device)
        pad_syn_i = torch.zeros(batch, T_data + self.T_no-1, self.I_no).to(self.device)
        ...
        layer1_e_kern = torch.flip(layer1_e_kern, [1]).reshape(self.hid_no, self.E_no, self.T_no)
        layer1_i_kern = torch.flip(layer1_i_kern, [1]).reshape(self.hid_no, self.I_no, self.T_no)
