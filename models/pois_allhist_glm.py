...
class Pois_AllHist_GLM(nn.Module):
    def __init__(self, C_den, C_syn_e, C_syn_i, T_no, device):
        ...
        self.W_syn = nn.Parameter(torch.ones(self.sub_no, 2)*(-5) , requires_grad=True)
        #self.W_syn = nn.Parameter(torch.randn(self.sub_no, self.cos_basis_no, 2)-6 ,  requires_grad=True)
        ...
        ### Output Parameters ###
        self.Theta = nn.Parameter(torch.ones(self.sub_no)*(-6) , requires_grad=True)
    ...
    def spike_convolve(self, S_e, S_i):
        ...
        #e_kern = torch.matmul(torch.exp(self.W_syn[:,:,0]) , self.cos_basis)
        #i_kern = torch.matmul(torch.exp(self.W_syn[:,:,1])*(-1) , self.cos_basis)
    ...
    def train_forward(self, S_e, S_i, Z):
        ...
        P = torch.exp(syn + self.Theta.reshape(1,-1) + hist_filt + spk_filt)
    ...
    def test_forward(self, S_e, S_i):
        ...
        for t in range(T_data):
            ...
            P[t] = P[t] + torch.exp(sub_in)
            Z[t+self.T_no] = Z[t+self.T_no] + torch.poisson(torch.exp(sub_in))
...
class GLM_Encoder(nn.Module):
    def __init__(self, C_syn_e, C_syn_i, T_no, device):
        ...
        ### Synaptic Parameters ###
        self.Tau_syn = nn.Parameter(torch.ones(self.sub_no-1, 2)*2 , requires_grad=True)
        self.Delta_syn = nn.Parameter(torch.zeros(self.sub_no-1, 2) , requires_grad=True)
        self.W_syn = nn.Parameter(torch.ones(self.sub_no-1, 2)*(-4) , requires_grad=True)
        
        #self.W_syn = nn.Parameter(torch.randn(self.sub_no-1, self.cos_basis_no, 2)-6 ,  requires_grad=True)
        
        self.Theta = nn.Parameter(torch.ones(self.sub_no-1)*(-6), requires_grad=True)
