...
class DetAE_AllHist_GLM(nn.Module):
    def __init__(self, C_den, C_syn_e, C_syn_i, T_no, device):
        ...
        #self.W_hist = nn.Parameter(torch.ones(self.sub_no, self.cos_basis_no)*(-2) , requires_grad=True)

        ### Output Parameters ###
        self.Theta = nn.Parameter(torch.ones(self.sub_no)*(-0.5) , requires_grad=True)
    ...
    def train_forward(self, S_e, S_i, Z):
        ...
        hist_in = F.conv1d(Z_pad, hist_kern, groups=self.sub_no).squeeze(0).T[:-1]
        spk_in_raw = F.conv1d(Z_pad, spk_kern, groups=self.sub_no).squeeze(0).T[:-1]
        spk_in = torch.matmul(spk_in_raw, self.C_den.T)

        Z = self.step(syn + hist_in + spk_in + self.Theta.reshape(1,-1))
        P = torch.sigmoid(syn + hist_in + spk_in + self.Theta.reshape(1,-1))
...
class GLM_Encoder(nn.Module):
    def __init__(self, C_syn_e, C_syn_i, T_no, device):
        ...
        ### Synaptic Parameters ###
        self.Tau_syn = nn.Parameter(torch.zeros(self.sub_no-1, 2) , requires_grad=True)
        self.Delta_syn = nn.Parameter(torch.zeros(self.sub_no-1, 2) , requires_grad=True)
        self.W_syn = nn.Parameter(torch.ones(self.sub_no-1, 2)*(-2) , requires_grad=True)
                
        ### Output Parameters ###
        self.Theta = nn.Parameter(torch.ones(self.sub_no-1)*(0) , requires_grad=True)
        
        ### Obs Basis ###
        self.obs_basis_no = 17
    ...
    def forward(self, Z, S_e, S_i):
        ...
        Z_out = self.step(syn + obs_filt + self.Theta.reshape(1,-1))
        P_out = torch.sigmoid(syn + obs_filt + self.Theta.reshape(1,-1))
