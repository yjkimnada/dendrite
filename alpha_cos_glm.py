...
class Alpha_Cos_GLM(nn.Module):
    def __init__(self, C_den, E_no, I_no, T_no, greedy, C_syn_e, C_syn_i, device):
        ...
        self.hist_weights = nn.Parameter(torch.randn(self.sub_no, self.cos_basis_no)*0.05 , requires_grad=True)
    ...
    def forward(self, S_e, S_i, temp, test):
        ...
        sub_out = torch.zeros(T_data+self.T_no, self.sub_no).cuda()
